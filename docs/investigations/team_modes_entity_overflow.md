...
## Fix
Before cloning the new helpers we explicitly free any lingering `frontBounds`/`rearBounds` entities attached to the body queue slot. This keeps the helper count bounded by the body queue size and matches the behaviour from the 0.5c release.
